...
<div class="page">
  ...
  <div class="paper-title">
    ...
    <div class="affil">Department of Computer Science and Engineering</div>
    <div class="email">icedevil196@gmail.com</div>
  </div>
  ...
  <div class="two-col">
    ...
    <p>
      Local-first software [5] offers an architectural approach where the primary data store resides on the client device, eliminating friction of account creation and enabling immediate, zero-latency interaction. This approach is particularly suitable for prototyping and student projects where backend infrastructure may not be available.
    </p>
    ...
    <p>3) To support multi-project workspaces with team collaboration features within a local-first browser architecture.</p>
    ...
    <p>
      Kleppmann et al. [5] introduced the local-first software paradigm, arguing that applications should prioritize local data ownership while supporting optional sync. Their principles—fast, offline-capable, owned data—directly shaped KarnBoard's localStorage-first architecture. The authors note that local-first applications offer superior responsiveness and privacy, at the cost of cross-device sync complexity.
    </p>
    ...
    <p>
      KarnBoard is implemented as a React 18 SPA bundled with Vite 8 and styled with Tailwind CSS v3. The application runs entirely in the browser; there is no server-side component or network requirement after initial page load. All application state is persisted using the Web Storage API (localStorage), with automatic migration support for users upgrading from a single-board to multi-board data format.
    </p>
    <p>
      The data model centers on a <em>boards</em> array, where each board object contains: <em>tasks[]</em>, <em>members[]</em>, <em>activity[]</em>, <em>messages[]</em>, <em>moodboard[]</em>, and scalar fields for XP, streak, phase, and description. The active board is tracked by ID in a separate localStorage key and mirrored to a React ref to prevent stale closure issues in memoized callbacks.
    </p>
    <p>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>No Login Required</td><td>✗</td><td>✗</td><td>✓</td></tr>
    ...
    <p>3) A local-first multi-project architecture with automatic single-to-multi-board data migration.</p>
    ...
    <p>
      The current implementation has several limitations. Real-time multi-user synchronization is not supported; data exists only on the local device. The localStorage API imposes an approximate 5 MB storage limit. The mobile drag-and-drop experience requires improvement. The SUS evaluation involved a small sample (n=5), limiting statistical generalizability.
    </p>

    <h3 class="alpha" data-letter="D">Future Work</h3>
    <p>
      Planned future work includes: (1) a backend API with WebSocket support for real-time multi-device sync; (2) a longitudinal user study measuring productivity impact over a 4-week period; (3) a mobile-native tap-to-move fallback for Kanban columns on touch devices; (4) project rename functionality and sprint/milestone grouping; (5) AI-assisted task suggestion based on project phase and team skills.
    </p>
